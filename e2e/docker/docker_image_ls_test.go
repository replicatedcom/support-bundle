...
specs:
  ...
  - docker.images:
    ...
    output_dir: /docker/images/`)
			...
			_ = GetResultFromBundle("docker/images/image_ls.raw")
			contents = GetFileFromBundle("docker/images/image_ls.raw")
			Expect(contents).To(ContainSubstring("RepoTags"))
			...
			err = json.Unmarshal([]byte(contents), &m)
			Expect(err).NotTo(HaveOccurred())
			_ = GetResultFromBundle("docker/images/image_ls.human")
			contents = GetFileFromBundle("docker/images/image_ls.human")
			Expect(contents).To(ContainSubstring("RepoTags"))
		})
	})
})

...
func (d *Docker) ContainerRun(spec types.Spec) []types.Task {
	...
	if spec.DockerContainerRun.ContainerCreateConfig.Config == nil {
		err = errors.New("spec for docker.container-run options ContainerCreateConfig.Config required")
	} else if spec.DockerContainerRun.ContainerCreateConfig.Config.Image == "" {
		err = errors.New("spec for docker.container-run options ContainerCreateConfig.Config.Image required")
	}
	if err != nil {
		task := plans.PreparedError(err, spec)
		return []types.Task{task}
